    ...
    <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-slate-200/50 shadow-sm ring-1 ring-slate-900/5 flex flex-col items-center text-center">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Incoming Assets</span>
          <span className="text-2xl font-black text-slate-900 tracking-tighter">{summary.received}</span>
          <span className="text-[9px] font-bold text-blue-600/60 uppercase tracking-widest mt-1">Total Pieces</span>
        </div>
        <div className="bg-white p-5 rounded-3xl border border-slate-200/50 shadow-sm ring-1 ring-slate-900/5 flex flex-col items-center text-center">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Stock Released</span>
          <span className="text-2xl font-black text-slate-900 tracking-tighter">{summary.disbursed}</span>
          <span className="text-[9px] font-bold text-slate-900/40 uppercase tracking-widest mt-1">Total Pieces</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {periodItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-center space-y-4 bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm opacity-60">
             <BarChart3 className="w-10 h-10 text-slate-200" />
             <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">No activity in this period</p>
          </div>
        ) : (
          periodItems.map(item => (
            <div key={item.id} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm ring-1 ring-slate-900/5 transition-all hover:shadow-xl hover:shadow-slate-200/30 hover:border-slate-300">
              <div className="flex justify-between items-start mb-4">
                <div className="space-y-0.5">
                  <h3 className="font-black text-lg text-slate-900 tracking-tight leading-tight">{item.name}</h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{item.itemType} • {item.baseUnit}</p>
                </div>
                <div className="bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                  <span className="text-[9px] font-black text-slate-900 uppercase tracking-widest">{item.transactions.length} Ops</span>
                </div>
              </div>

              <div className="space-y-2">
                {item.transactions.map(tx => (
                  <div key={tx.id} className="flex justify-between items-center bg-slate-50/50 p-3 rounded-xl border border-slate-100/50 group hover:bg-white hover:border-slate-200 transition-all">
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-bold text-slate-400 whitespace-nowrap">{formatDateTimePHT(tx.date)}</span>
                      </div>
                      <span className="text-[9px] font-bold text-slate-600 mt-0.5 truncate max-w-[140px]">
                        {tx.receiverId ? `To: ${getReceiverName(tx.receiverId)}` : `Batch: ${tx.batchNumber || 'N/A'}`}
                      </span>
                    </div>
                    <div className="flex flex-col items-end shrink-0 ml-3">
                      ...
                      <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-0.5">{tx.pieceQuantity.toFixed(2)} {item.baseUnit}s</span>
                    </div>
                  </div>
                ))}
              </div>
